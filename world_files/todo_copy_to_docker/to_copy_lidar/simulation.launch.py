...
def generate_launch_description():
    ...
    use_simulated_lidar = LaunchConfiguration('use_simulated_lidar', default='false')
    ...
    declare_use_simulated_lidar = DeclareLaunchArgument(
        'use_simulated_lidar',
        default_value='true',
        description='Use simulated VLP 16 LIDAR if true'
    )
    use_robot_state_pub = LaunchConfiguration('use_robot_state_pub')
    if use_simulated_lidar:
        urdf = os.path.join(get_package_share_directory('neo_simulation2'), 'robots/'+MY_NEO_ROBOT+'/', MY_NEO_ROBOT+'_with_lidar.urdf')
    else:
